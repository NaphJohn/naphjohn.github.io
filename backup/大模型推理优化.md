...
GPU芯片架构：
GPU 采用SIMT 架构，拥有数千个计算核心，适合大规模并行计算。核心组件包括SM 流式多处理器、CUDA Core和Tensor Core。内存层次从寄存器到显存，速度递减容量递增。相比 CPU，GPU 在矩阵运算和 AI 训练上有 10-100 倍性能优势，主要得益于高并行度和专用计算单元。NVIDIA 最新架构如 Hopper 还引入了Transformer Engine专门优化大模型训练。
