...
原理：离线地、逐通道地把激活值中的离群值‘抹平’，把因此产生的数值范围膨胀等价地迁移到静态权重上，使得激活和权重都能用 INT8 量化而几乎不掉点
...
“16-token页式KV+算子重排的方式优化，短序列长尾显存-15 %，吞吐+8 %，碎片归0。”
W8A8C8优化项目三：
...
改成 [B,S,H,D] 后，同一个线程束（Warp）的 32 个线程**按 seq 维连续编号**：
线程 0 读 addr
