...
精准探测硬件拓扑。
强制绑定进程到正确的NUMA节点。
实现 locality，让计算（CPU）、数据（内存）和设备（NPU）三者尽可能靠近。
...
```shell
# 1. 获取NPU Bus-ID (例如：0001:74:00.0)
npu-smi info

# 2. 查询该设备的NUMA节点
lspci -vs 0001:74:00.0 | grep NUMA
# 或者使用更通用的方法：
cat /sys/class/ascend_npu/devices/0001\:74\:00.0/device/numa_node
...
# 启动 Rank 1， 绑定到 NUMA Node 1， 使用 NPU 1
numactl --cpunodebind=1 --membind=1 \
    python my_training_script.py --rank 1 --device 1
命令解释：
...
EP专家并行：在 MoE 模型中，把“专家”切分到不同设备，token 通过 All-to-All 通信“去找”专家，计算完再 All-to-All 回来。
...
<details><summary>Details</summary>
<p>
让 vLLM 的 OpenAI-Compatible 服务在 流式/非流式 场景下都能把“思考过程”独立透出。
...
主要逻辑图
extract_reasoning_content(model_output, request)
├─ 1. 快速过滤：不含任何标记 → return (None, model_output)
├─ 2. 去掉 start_token 及其左侧垃圾
│   model_output = model_output.partition(start_token)[2]
├─ 3. 再次 partition(end_token) 得到三元组
│   (reasoning_content, end_token, content)
├─ 4. 若 content 为空串 → 设为 None
└─ 5. return (reasoning_content, content)

extract_reasoning_content_streaming(delta_text)
├─ 每次只收到增量 delta_text
└─ 直接包成 DeltaMessage(reasoning_content=delta_text)
（说明：流式场景由外层逐段调用，本函数只负责“封装”，不重新做切分）

非流式
model_output = "aaaa<think>reason</think>final answer"
→ (reasoning_content="reason", content="final answer")

流式：
delta_text="rea"  → DeltaMessage(reasoning_content="rea")
delta_text="son"  → DeltaMessage(reasoning_content="son")
delta_text="</think>" → DeltaMessage(reasoning_content="</think>")
delta_text="final" → DeltaMessage(content="final")

思维链长度控制：

| 场景                | 开关=0（默认）                    | 开关=1                                |
| ----------------- | --------------------------- | ----------------------------------- |
| 思维链 40 k          | 触发 max\_tokens 40 k 上限 → 截断 | 触发 REASONING\_MAX\_TOKENS 32 k → 截断 |
| 思维链 20 k + 正文 9 k | 总输出 29 k ≥ 32 k → 截断        | 正文 9 k ≥ 8 k → 截断                   |
| 思维链 20 k + 正文 7 k | 通过                          | 通过                                  |
