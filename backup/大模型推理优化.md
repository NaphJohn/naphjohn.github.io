...
量化步骤（四步流程）
1. 旋转参数化
在 Transformer 块内插入 4 类正交旋转矩阵 R₁～R₄（见图）
R₁/R₂ 与权重合并（离线），R₃/R₄ 在线作用于激活 & KV-Cache
所有 R 满足 R Rᵀ = I，因此 Y = X W ≡ (X R)(Rᵀ W)，输出数值不变
2. 学习旋转
冻结原权重，仅优化 R
用 Cayley SGD 在 Stiefel 流形上更新，保证始终正交
损失 = 旋转 + 4-bit 量化后 vs 全精度输出的 MSE
训练 30 min（7B）～3.5 h（70B）自动收敛
3. 分段校准选量化点
权重：128×128 超级块 → 32×32 子块，MSE 最优对称缩放 s（z=0）
激活/KV：1×128 行级动态量化，每行实时算 s
两级缩放因子 → 硬件张量核友好
4. 4-bit 部署
权重 W4，激活 A4，KV-Cache INT4
推理时 R₃/R₄ 用 Hadamard（±1）零乘实现，额外延迟 <1 %

优势：
精度高：在w4a4/kv4全4bit场景刷新PTQ记录；
鲁棒性好：学习选装方差小；
兼容性强：可叠加GPTQ、AWQ
推理延迟低：离线旋转零代价
...
**awq量化**：识别重要权重--找最优缩放因子--量化重建。AWQ 用「校准集平均幅值 + 20 点 α 网格搜索」得到 per-channel 向量 s，只放大显著通道的权重，把量化误差从激活赶到权重，再靠 4 bit 权重吸收掉
awq与smoothquant量化区别：
<details><summary>Details</summary>
<p>
awq:  量化点只对「输入通道」维度做 per-channel 量化，且只保护 ≈0.1% 的显著通道;
缩放因子 s 怎么来:
以「输入校准集在该通道的平均绝对值」sx 当成显著性指标，再引入一个 0≤α≤1 的超参做折中：
s = sx^α      （逐通道向量）
α=0  → 不缩放；α=1  → 最激进放大显著权重。
由于量化函数不可导，作者用 网格搜索 找最优 α：在 [0,1] 区间等距取 20 个点，对每一 α 计算整层输出 MSE，挑损失最小的 α*，从而确定最终 s 

|      | AWQ              | SmoothQuant                   |
| ---- | ---------------- | ----------------------------- |
| 量化精度 | W4A16            | W8A8                          |
| 缩放对象 | 仅显著权重（~0.1% 通道）  | 所有权重+激活（100% 通道）              |
| 缩放因子 | sx^α（α 搜 20 点）   | sx^α / sw^(1−α)（α 经验）         |
| 搜索方式 | 网格搜最小输出 MSE      | 闭式公式 + 小范围扫 α                 |
| 硬件粒度 | per-channel 权重   | per-token 激活 + per-channel 权重 |
| 推理开销 | 零（scale 已融进量化参数） | 零（同样融合）                       |
...
常用的一些数值：
NRMSE 看整体误差，Skewness/Kurtosis 找离群，Energy_ratio 找主通道，Radio/MaxTail 给 SmoothQuant 放大系数
</p>
</details>
