...
### 量化项目一：  低精度解决幻觉问题
主流方案：量化回退w8a8/awq +回退脚本  或者是 旋转量化+层 / Token 级 FP16 回退
...
1. 离线预筛：一次性扫描权重，用离散度 R 【 (最大值-最小值)/(上四分位-下四分位) 】和 KL 误差
KL 误差 = 量化前后该层输出分布之间的 KL 散度（Kullback-Leibler divergence）。
计算步骤

- 用一小批校准数据（通常 512~2048 条）跑 FP32 模型，收集该层输出特征图，得到真实分布 P。
- 用同一批数据跑 伪量化版本（权重按候选方案量化为 4/8 bit 后再反量化），得到近似分布 Q。
- 逐通道统计直方图，计算
- KL(P‖Q) = Σ P(i) log(P(i)/Q(i))
- 值越大 → 量化后分布越偏离原分布，该层对量化越“敏感”。

把每层标成“可激进量化 / 需 per-channel / 必须 FP16”三档，写进 sensitive_layers.json。
