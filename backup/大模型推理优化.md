...
空调度时 _dummy_run(1) 保同步，单机 sleep(0.0001) 放 GIL，防止分布式死锁；（CPython 解释器全局锁；保证同一时刻只有一个线程在执行 Python 字节码；多线程无法真正并行，只并发；CPU 密集任务仍受限于单核）
