...
总结一句话是：GPU 是通用并行处理器，灵活性强，适合图形+AI；NPU 是专用AI加速器，能效比高，专为神经网络设计。
...
GPU芯片架构：
GPU 采用SIMT （单指令多线程）架构，拥有数千个计算核心，适合大规模并行计算。核心组件包括SM 流式多处理器、CUDA Core和Tensor Core。内存层次从寄存器到显存，速度递减容量递增。相比 CPU，GPU 在矩阵运算和 AI 训练上有 10-100 倍性能优势，主要得益于高并行度和专用计算单元。NVIDIA 最新架构如 Hopper 还引入了Transformer Engine专门优化大模型训练。

架构演进：

Volta | 2017 | 首次 Tensor Core
-- | -- | --
Turing | 2018 | RT Core + Tensor Core
Ampere | 2020 | 第三代 Tensor Core
Hopper | 2022 | Transformer Engine
Blackwell | 2024 | 第二代 Transformer Engine
