...
1. 从框架测（量化，其它特性）
2. 压缩内存，kv cache角度
3. 算子角度，提升通信算子，算子融合重排等方式来提升吞吐。
4. 优化算法，比如PD分离，图模式，张量并行，数据并行，序列并行这些方式
...
ascend_vllm/v1/core/sched/scheduler.py
ascend_vllm/v1/worker/model_runner_v1.py
1. 在KV消费者模式（多个请求共享和复用KV Cache内存空间）下，从头开始调度以确保KV Cache的高效利用
2. 预算感知调度
基于token预算进行调度，确保计算资源不被过度分配
动态调整调度数量，避免资源浪费
3. 防重复调度检查
4. 资源边界保护
强制实施运行请求数量上限，防止资源过载
在非KV消费者模式下验证调度总数不超过运行中请求数，确保调度逻辑正确性
5. 空调度以及分布式优化
空调度时候：
数据并行环境 (world_size > 1)，运行一个虚拟的前向传播（_dummy_run(1)：）；（保持分布式进程间的同步，避免死锁）；
单机环境，短暂释放GIL（time.sleep(0.0001)）；
kv cache检查：如果没有KV传输组，直接返回空结果
6. 故障快速隔离与资源即时回收：
- 立即识别并隔离KV Cache传输失败的请求
- 立即释放失败请求占用的KV Cache内存
- 给客户端明确的完成状态，而不是超时或无响应EngineCoreOutput( new_token_ids=[1], # 返回明确的状态标识）
- 保持调度队列清洁，只包含可正常处理的请求
7. 长序列 PD 分离场景下的 fast-fail 优化
- 给 vLLM 增加了一种新的提前失败 finish_reason状态：“pd_link_down”，当分布式推理里“拉不到 KV（Pull KV 失败）”即判定链路掉线，立刻终止生成并回 500，省掉无谓重试与显存占用（打了补丁，vllm/entrypoints/openai/serving_chat.py）；
- “PD 分离场景下 KV-拉取失败”从无限重试改成可配置次数 + 单次超时 + 立即登记失败 ID；上层拿到失败列表后立刻触发 pd_link_down fast-fail，砍掉长尾重试、节省显存与调度槽位。
8. 通信算子优化：优先走 昇腾 HCCL 专用 all-to-all 算子（hcclAlltoAll），省掉 PyTorch 通用路径的 view-flatten + 额外空 buffer，消除一次内存拷贝与形状变换开销
9. NZ格式优化（支持qkvo-matmul nz）
ascend_vllm/model_executor/layers/quantization/compressed_tensors/schemes/compressed_tensors_w8a8_int8.py
layer.weight.data = torch_npu.npu_format_cast(weight.data.t().contiguous(), FORMAT_FRACTAL_NZ)
layer.antiquantized_weight.data = torch_npu.npu_format_cast(antiquantized_weight.t(), FORMAT_FRACTAL_NZ)
10. 避免大张量concat操作的内存开销，切分长序列下的激活，减少峰值显存占用
<details><summary>Details</summary>
<p>
长序列场景 (N >= MOE_SEQ_SPLIT_LENGTH):
传统: 创建 [N, 2*H] 的大张量 → 高内存峰值
优化: 分别处理两个 [N, H] 张量 → 内存减半
moe中vllm_npu/fx/model_executor/models/deepseek_v3_mtp.py
if tok_embeds.shape[0] >= MOE_SEQ_SPLIT_LENGTH:
    # 长序列：使用split优化路径，避免大张量concat
    inputs_embeds = self.eh_proj.forward_split(tok_embeds, previous)
else:
    # 短序列：正常concat路径
    cat_hidden_states = concat([tok_embeds, previous], -1)
    inputs_embeds = self.eh_proj.forward(cat_hidden_states)
</p>
</details> 
11. APC中oom修复
修复apc中oom，使用prefix cache功能时，当前每层都会新建一个pa out tensor，并保存在类成员变量中，不及时释放，导致oom，如图所示；并且在实际场景中，输入长度几乎不可能和上一次一致，所以其实每次都需要重新创建tensor，基本没有复用的场景。
vllm_npu/common/attention/backends/fx_base_attn.py
pa_out = torch.empty_like(query)
12. TP+SP（Tensor + Sequence Parallel）：把 128 k 序列按 2k-4k token 切片到多卡，每卡 Attention 降到 (seq/TP)²，TTFT 随卡数线性下降（Megatron-LM 实测 8 卡↓8×）。
...
### flash attention
原理：针对全局内存和共享存储的 I/O 速 度的不同，尽可能的避免 HBM 中读取或写入注意力矩阵（N×N 的 S/P 矩阵）。FlashAttention 目标是尽可能高效地使 用 SRAM 来分块计算，避免从全局内存中读取和写入注意力矩阵。
...
chunk-prefill--提升TTFT：
1. 调度器模块 (scheduler/)
2. 请求状态管理，保存块间kv cache
3. 模型执行器
4. kv cache管理， # 增量更新KV Cache，而不是重新分配
