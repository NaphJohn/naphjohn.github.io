...
## Stablediffusion对应项目
"关于蛋白质生成模型，目前主流是扩散模型如 RFdiffusion，
支持按功能约束直接生成 3D 结构，已成事实标准。

而我们项目聚焦小分子生成，通过'生成 - 评估 - 优化'闭环提升蛋白质药物研发效率：
1. 用 Stable Diffusion 框架生成 3 万+ 候选分子，有效性 0.984；
2. 用 XGBoost 替代分子对接（计算小分子（药物候选）与靶点蛋白（如酶、受体）的3D 结合模式 + 结合亲和力的模拟方法。），初筛效率提升 100 倍，准确率 94.92%；
3. 创新融入粒子群优化，在潜空间定向搜索，靶点分数从 9.3 提升至 10.20；
4. 湿实验反馈闭环，研发周期从 6 月缩短至 2 月。

核心思路是：用生成模型扩大搜索空间，用快速评估缩小候选集，
用优化算法定向提升，三者协同实现效率与质量的双重突破。"

Xgboost：
药物分子和蛋白的结合通常被转化为固定长度的特征向量（如指纹、描述符、相互作用计数）。这是典型的表格数据。
使用 XGBoost 是因为它在表格型构效关系数据上具备极高的推理速度和优秀的泛化能力。它通过将复杂的 3D 结合问题抽象为特征向量，利用二阶泰勒展开精准拟合非线性关系，并利用正则化克服实验数据噪声，从而实现了在保持高准确率（94.92%）的前提下，将筛选效率提升了两个数量级。
注意：这种方案通常用于初筛（Primary Screening）。通过 XGBoost 筛选出的 Top 1% 分子，通常还需要再用传统的分子对接或自由能微扰（FEP）进行二次验证，以确保 3D 结合模式的合理性。
...
文本 → T5 编码 → STDiT 逐帧去噪 → 时空 VAE 解码 → 视频文件（mp4, 25 fps）
「先压后去噪」
VAE 把视频压成隐码 → DiT 在隐码里玩 Diffusion → 推理用最终权重解码回视频
「OpenSora = 时空 VAE + STDiT（时空扩散变换器，用于在视频生成中同时对空间维度（单帧图像）和时间维度（多帧时序）进行建模与去噪生成。） + T5；WebVid-2M 学；VAE loss ↘ 0.8e-3，DiT loss ↘ 0.05 停；输出 4-16 s 720p 视频。」
opensora模型与SD模型架构比较
...
结构：在 DiT 基础上 串行叠加
① Spatial-Attention（帧内 2D）→ ② Temporal-Attention（帧间 1D）→ ③ Cross-Attention（接收 T5 文本嵌入）
输入：T 帧噪声潜码 + T5 文本嵌入
输出：T 帧去噪潜码（保持时序连贯）
优点：相比全注意力，计算量降低且能直接复用预训练图像 DiT 权重做视频迁移
...
## qewn2.5 VS qwen3-vl
从输入到输出过程：
高清图像被切分为动态 Patch，经 ViT 提取特征后，通过 Projection 转化为视觉 Token，与文本 Token 一起注入 M-RoPE 获得时空坐标感，最终送入 Qwen3 LLM 进行深度推理，以自回归方式输出文本、坐标或代码。
...
一句话定位
用 一套参数 同时支持文本、图像、音频、视频四模态输入，并实时输出 文本 + 自然语音。
核心架构——Thinker-Talker MoE
Thinker：30 B-MoE，128 专家/激活 8 专家，负责多模态理解、推理与文本生成；
Talker：独立 MoE，以 Thinker 的高级语义为条件，专注 流式语音合成；
两模块 解耦设计，可分别配置系统提示，实现“说什么”与“怎么发音”的细粒度控制。
四模态统一流程
文本/图像/音频/视频 → Vision Encoder + AuT 音频编码器 → 统一时间 ID 位置编码 → Thinker MoE → 文本 token + 隐状态 → Talker MoE → 多码本语音帧 → Code2Wav 因果卷积 → 16 kHz 波形流式输出，首包延迟 211 ms。
关键优化
多码本自回归：每步只产 1 个核心帧，MTP 并行预测剩余残差，降低首包等待；
早期混合训练 + 模态掩码：保证单模态性能不下降，避免“多模态诅咒”；
MoE + 分块预填充：长序列 KV-Cache IO 减少 40%，并发量提升 3×
