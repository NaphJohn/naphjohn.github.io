...
## 训练中资源规划
算力资源规划针对不同模型，集群规格（Snt9b、Snt9b23）、数据集大小等均会导致训练时间、集群卡数规模不同，如训练过程中对卡数或训练时间有要求，可根据以下公式计算预测：
训练时间(秒)：Time=Token总/(TPS*N卡数)*线性加速比，计算出的训练时间为范围值，仅作参考。
训练卡数：N卡数=Token总/(Time*TPS)*线性加速比，N卡数>8时则需取整为8的倍数且不小于模型最小卡数配置。
参数解释：
总Tokens
...
一般的话会预留整体5%资源量的Snt9b23资源做热备。

# k8s
k8s =「apiserver + etcd + 控制器 + 调度器」控制面，「kubelet + kube-proxy」数据面，声明式循环保终态一致。

用户写 YAML → apiserver 入库 → 控制器持续对比「期望 vs 实际」→ 调度器绑定 Node → kubelet 调 CRI 启动容器 → 健康探针/状态回写 → 循环直至终态一致。

具体原理以及Pod/InfiniBand/RDMA原理如下：
...
| 组件 | 原理一句话 |
|---|---|
| kube-apiserver | RESTful 前端，唯一入口；所有请求/状态先写 etcd |
| etcd | 分布式 KV 库，单源真相；存集群所有对象期望与实际状态 |
| kube-controller-manager | 运行 N 个控制器循环，对比期望与实际，不断调谐直至一致 |
| kube-scheduler | 给 Pod 打分选 Node，绑定后写入 etcd |
2. 数据面（Node）——「手脚」
| 组件 | 原理一句话 |
|---|---|
| kubelet | 节点级 Agent，监听本机 PodSpec；调 CRI 启停容器，上报状态 |
| kube-proxy | 维护iptables/IPVS 规则，实现 ClusterIP / NodePort 负载均衡 |
3. 插件层（非必需但标配）
CNI 插件：给 Pod 插网线（Calico、Cilium 等）
CSI 插件：给 Pod 挂磁盘（ceph, EBS 等）
CoreDNS：集群内 DNS 解析
Metrics-Server：HPA 用资源指标

Pod
是 K8s 的最小可调度单元，里面封装一个或多个容器、共享网络/存储，声明即可把应用（训练脚本、推理服务等）跑在任意节点。
InfiniBand
一种高速、低延迟（< 1 µs）的“独立网络”，专为 RDMA 设计；在 K8s 中通过 SR-IOV 把物理网卡切成 VF，再用 Device Plugin 把 VF 注册成可调度资源 rdma/hca，Pod 声明后就能直通使用。
RDMA
Remote Direct Memory Access——网卡直接读写远端内存，零拷贝、内核旁路；配合 GPUDirect RDMA 可实现 GPU 显存跨机 DMA，极大提升分布式训练/推理带宽，延迟远低于 TCP/IP。

</p>
</details> 

# 训练框架
AscendFactory是对多种训练框架的一个封装，当前支持Llama-Factory、MindSpeed-LLM（原ModelLink）、VeRL、MindSpeed-RL和MindSpeed-MM五种训练框架作为后端，提供一键式训练。训练性能主要通过训练日志中的2个指标查看：吞吐量和收敛情况。
...
| 产线        | 一句话记忆               | 吞吐量（完整参数名）                                   | 收敛参数                        | 平稳阈值（经验）  |
| --------- | ------------------- | -------------------------------------------- | --------------------------- | --------- |
| **MM**    | 图文对齐，看 loss 降       | `GBS/elapsed*1000` (samples/s)               | loss                        | < 0.01 波动 |
| **LLM**   | 大模型预训练，看 lm\_loss 降 | `GBS×SEQ_LEN/(卡数×elapsed)*1000` (tokens/s/p) | lm\_loss                    | 下降→平稳     |
| **RL**    | 强化微调，看 reward 升     | `e2e_tps/update_tps/vllm_tps` (tps)          | grpo/xxx\_acc\_rewards/mean | > 0.8 且平稳 |
| **LLM-F** | 轻量 LoRA，看 loss 降    | `delta_tokens/delta_time/卡数` (tokens/s)      | loss                        | 下降→平稳     |
| **VeRL**  | 强化对齐，看 critic 升     | `perf/throughput` (tps)                      | critic/score/mean           | →1 且平稳    |


面试速记：
tps = 每秒迭代/更新/采样次数，perf = 每秒平均样本数，二者都是越大越好

不同版本对应特性：
相较于6.5.908版本，6.5.910有以下新增支持特性：VeRL强化学习框架：新增异步rollout、工具调用功能；新增训推分离功能one-step off-policy；新增支持DAPO/GRPO中LoRA微调；精度工具支持内存snapshot和精度数据采集解耦；精度工具支持Mock推理功能中NPU和GPU数据随机保序采样重用。MindSpeed-RL框架：新增支持填充移除（Remove Padding）。MindSpeed-MM框架：新增支持Qwen2.5VL- 7B、InternVL3-8B模型的预训练和微调。MindSpeed-LLM框架：Qwen2.5-7B/32B/72B支持32K长序列。

相对于6.5.907版本，6.5.908有以下新增支持特性：VeRL强化学习框架：新增Qwen2.5-VL-72B DAPO、Qwen3-30B-A3B GRPO/DAPO、Qwen2.5-72B GRPO/DAPO；新增支持Megatron训练后端；32k长序列：Qwen2.5-72B、Qwen3-8B、Qwen3-32B、Qwen2.5-VL-7B；新增精度数据采集工具。Llama-Factory框架：新增支持长序列并行（Ring Attention、Ulysses）；32k长序列场景：Qwen3-8B、Qwen2.5-VL-7B、Qwen2.5-VL-32B的Full微调；MindSpeed-LLM框架：32k长序列，Qwen2.5-0.5B/1.5B/7B/14B/32B/72B、Qwen3-0.6/1.7B/4B/14B/32B、Qwen3-30B-A3B；MindSpeed-RL框架：新增Qwen2.5-VL-3B、Qwen2.5-VL-7B、Qwen2.5-32B、Qwen3-32B 、Qwen3-30B-A3B模型的GRPO；32k长序列：Qwen2.5-32B。
...
DAPO/GRPO + Lora微调
就是只改 Adapter 不改主干的 RLHF：冻结 W₀， rollout→奖励→PPO 全部在 A·B 子空间完成；输出一个 10 MB 小权重，立刻上线新策略
...
冻结主干 W₀
   ↑
输入样本 s → 线性投影 → LoRA Adapter A·B → 策略 πθ(s)  
   ↑                                      ↓
奖励函数 r(s,a) ← 采样动作 a ← rollout（异步/async）  
   ↑                                      ↓
PPO/GRPO 损失 L(θ) 只含 A·B 参数 → 梯度更新 A·B  
   ↑
输出：新 Adapter 权重 A'·B'（W₀ 仍不变）
...
训练方法包含PT、SFT、DPO；deepseed zero并行（Zero 的核心就是把模型状态（权重、梯度、优化器状态）按行/列切成分片，让每张 GPU 只存一小部分；前向-反向计算时再通过 All-Reduce / Broadcast / All-Gather 把需要的片段临时凑齐，用通信换显存。）；flashattention
...
### PPO与DPO区别
这两者都是用于 LLM 人类对齐（RLHF）的核心算法，但它们的优化范式和工程成本有本质区别。
第一，架构复杂度不同。
PPO 是经典的强化学习算法，它需要维护四个模型（Actor, Critic, Reward, Reference），并且在训练过程中需要进行在线采样和复杂的 Importance Sampling 计算。这导致它的显存占用极高，工程实现非常复杂，且超参数极其敏感，训练容易不稳定。
第二，DPO 做了数学上的简化。
DPO 的核心贡献在于它推导出最优策略和奖励函数之间存在解析解关系。因此，它不需要显式地训练 Reward Model，也不需要 Critic 网络。它将强化学习目标直接转化为了一个二元分类损失函数，直接在偏好数据对上进行优化。
第三，资源与稳定性。
在实际落地中，DPO 只需要两个模型（Policy 和 Reference），显存占用比 PPO 减少了一半以上。更重要的是，DPO 的训练过程像标准的 SFT 一样稳定，避免了 PPO 常见的模式坍塌（Mode Collapse）和调参困难问题。
总结来说，虽然 PPO 在理论上是 RLHF 的金标准，但在大多数文本生成任务中，DPO 能以极低的成本达到同等甚至更好的效果。因此，除非是涉及复杂环境交互（如游戏、机器人控制）必须用在线 RL 的场景，否则目前工业界首选 DPO 进行模型对齐。
