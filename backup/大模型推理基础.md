...
| **GQA** | 头分成 G 组，组内共享kv | G·n·d       | 质量≈MHA，显存-速度居中；GPT-4、Llama2 采用       |
...
MLA 的核心是通过 **低秩投影 (Low-Rank Projection)** 将 KV Cache 压缩到 **低维潜在空间 (Latent Space)**。

1. **写入时**：用 `W_down` 将高维隐藏状态投影到低维向量 `c`，**只缓存 `c`**。
2. **读取时**：用 `W_up` 将 `c` 还原成 **K 和 V**，再计算 Attention。
3. **优化**：配合 **解耦 Query (Decoupled Query)**，一部分 Q 从 `c` 还原，一部分直接从输入投影，**保证表达能力不损失**。
4. **优势**：相比 GQA，MLA 在 **显存占用更低** 的情况下，能保持接近 **MHA 的模型效果**，非常适合 **长上下文推理**。
...
**MOE原理**
**MoE** 是通过**稀疏激活**增加模型容量，一个共享专家，若干路由专家组成，由门控网络选择top-k个专家。：
- 多个 Expert FFN，每步只激活 Top-K 个
- 总参数大，但计算量≈dense 模型
