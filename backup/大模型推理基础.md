...
| 步骤         | 公式                     | 说明                         |
| ---------- | ---------------------- | -------------------------- |
| ① 路由决策     | `g = Softmax(W_r · x)` | 轻量级门控网络，输出 N 个专家的权重        |
| ② Top-K 选择 | `TopK(g, k=1~2)`       | 只保留最大 1~2 个值，其余置 −∞ → 稀疏门控 |
| ③ 专家计算     | `y_i = Expert_i(x)`    | 被选中的专家各自做标准 FFN（SwiGLU 等）  |
| ④ 加权合并     | `y = Σ g_i · y_i`      | 把激活专家的输出按门控权重加回残差流         |
实际部署：每隔 P 层（P=1,2,4…）把原来的 单层 FFN 换成 MoE 层即可，其余 Attention 保持不变

2. 为什么能“参数爆炸而计算不爆炸”
稀疏激活：每 token 只触达 k/N ≈ 5–10 % 参数
例：DeepSeek-V3 总 671 B，每次仅 37 B 激活 → FLOPs 降到稠密模型的 30 % 以下。
计算与参数解耦：增加专家数量 → 容量线性↑，延迟几乎不变（专家并行 + 预取）。
硬件友好：专家权重分片到不同 GPU，通信量 ≈ 门控向量 + 激活张量，远低于稠密模型 All-Gather。

3. 好处
| 好处                 | 数据佐证                                                                |
| ------------------ | ------------------------------------------------------------------- |
| **1. 容量↑ 计算成本≈常数** | 同 FLOP 预算下，MoE 版比 Dense 困惑度低 **10–15 %**；1 T 参数模型训练成本 ≈ 100 B 稠密模型。 |
| **2. 推理加速**        | 稀疏激活使 **首 token 延迟 ↓ 20–40 %**，吞吐 ↑ 2×（vLLM + 专家并行实测）。              |
| **3. 专家自动特化**      | 可视化显示：某些专家 80 % 流量来自代码 token，某些专吃中文，**减少任务冲突**→ 多任务精度 ↑。            |
| **4. 超长序列外推更容易**   | 相同计算预算可把层数/头维加大，**ROPE + 专家特化**在 64 k 上下文上仍保持低困惑度。                  |
...
## LLM 推理的性能卡点
三座大山：内存--调度--计算
“LLM 推理先撞内存墙，再撞调度墙，最后才轮得到计算墙。”
调优顺序：
把 KV-Cache 压下来（量化、分页、窗口注意力）→ 2. 把带宽跑满（融合 kernel、连续批）→ 3. 再把 CPU 调度开销藏好（CUDA Graph、重叠 Scheduler）。
...
| 子类               | 现象与量化数据                                                                   | 根因                           | 快速验证手段                                  |
| ---------------- | ------------------------------------------------------------------------- | ---------------------------- | --------------------------------------- |
| **KV-Cache 爆炸**  | Llama2-7B\@4k 场景，每并发需 ≈2 GB；A100-80G 理论只能跑 40 并发，实际 < 25                  | 自回归导致 cache 随 seq\_len 线性膨胀  | 观察显存随 batch×seq 线性增长                    |
| **显存带宽饱和**       | 7B fp16 模型在 14 ms/token 时需 1 TB/s，已吃满 A100 2 TB/s 的 50 %；MBU>0.4 即判定为带宽瓶颈 | 每生成 1 token 要把整个权重+cache 扫一遍 | `nvidia-smi dmon` 看内存利用率≈100 %而 SM<60 % |
| **PCIe/NUMA 搬运** | 多卡推理时 host→device 或卡间通信耗时占 TPOT 30 % 以上                                   | 张量并行引入 all-reduce            | nsys 显示 cudaMemcpyAsync 占比高             |
...
| 子类                     | 现象与量化数据                                                                     | 根因                   | 快速验证手段                 |
| ---------------------- | --------------------------------------------------------------------------- | -------------------- | ---------------------- |
| **动态批等待**              | 10 请求最长 10 s、最短 1 s，GPU 空转 9 s                                              | 请求级 batching 必须等最长序列 | 连续批处理可把吞吐提升 40 %       |
| **CPU launch 开销**      | CUDA kernel 每次由 Python 下发，launch 延迟 5-10 µs，在 1 ms 解码步占比 1 %，但千级并发即放大到 10 % | 框架即时调度               | Graph Capture 后降到 <1 % |
| **Context/Decode 相位差** | 首 token 时延（TTFT）与每 token 时延（TPOT）难兼顾，长 prompt 把计算资源吃光，decode 饥饿             | 两阶段算力争抢              | 分离调度或 chunked-prefill  |

3. 计算墙

| 子类           | 现象与量化数据                                                    | 根因                              | 快速验证手段                        |
| ------------ | ---------------------------------------------------------- | ------------------------------- | ----------------------------- |
| **GEMM 效率低** | 原生 PyTorch 120 ms/token → TensorRT-LLM 35 ms/token，降幅 3.4× | 未融合 QKV+Softmax+O 投影，kernel 碎片化 | nsight Compute 显示 SM 利用率<50 % |
| **量化反量化开销**  | IQ1\_M 在 Apple Silicon 上比 Q2\_K 慢 45 %，码本查找占 30 % 周期       | 低比特引入查表与位运算                     | 每 token CPU 周期数暴涨             |


MBU：Model Bandwidth Utilization（模型带宽利用率）
SM：“SM 利用率”≈ 这些核心在时钟周期内实际干活的比例，用来判断是否算力已榨干。
...
## vllm推理整体流程
V1 把「调度-执行-IO」拆成独立进程，用增量通信换掉全局广播，默认打开 chunk-prefill、前缀缓存等新优化，同等 GPU 下吞吐提升 10-30%，TTFT 降 30% 以上。
...
| 维度         | V0 现状                  | V1 改动                                                | 收益                         |
| ---------- | ---------------------- | ---------------------------------------------------- | -------------------------- |
| **进程模型**   | 单进程+AsyncLLMEngine 包装  | **EngineCore** 独立进程，**EngineCoreClient** RPC 调用      | 故障隔离、CPU-GPU 并行度↑          |
| **通信**     | 每步广播完整请求状态             | **只传增量 diff**（Δ-state）                               | 张量并行时通信量↓5-10×             |
| **调度器**    | 分 prefill / decode 两阶段 | **统一调度** `{req_id: #tokens}` 字典                      | 支持动态 chunk、推测解码无侵入         |
| **KV 缓存**  | `BlockManager` 中心化     | `KVCacheManager` 进程本地 + **零开销前缀缓存**                  | 默认开启，命中 0% 时性能下降 <1%       |
| **CPU 瓶颈** | 前端线程与 GPU 同进程          | **输入 Processor / 输出 Processor** 异步进程                 | 5 ms 级小模型 GPU 执行不再被 CPU 拖慢 |
| **连续批处理**  | 已实现                    | **更激进 chunk-prefill** 默认开                            | 长序列 TTFT 线性化，显存峰值↓30-40%   |
| **代码可维护性** | 2000+ 行 LLMEngine 单文件  | **模块化** SchedulerInterface、Processor、OutputProcessor | 新特性接入成本降低                  |
| **切换方式**   | 默认                     | `export VLLM_USE_V1=1`                               | 无缝回退（`=0` 即回 V0）           |
...
DSA：每个 token 动态选 1 个“最优单头” 计算，显存 ≈ 1；
