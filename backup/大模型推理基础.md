...
| 特性       | AllReduce     | AllGather           | AllToAllV             |
| -------- | ------------- | ------------------- | --------------------- |
| **通俗比喻** | 大家凑份子钱，每人知道总额 | 大家交换章节，每人拥有全书       | 大家互换扑克牌，按需分配          |
| **数据变化** | 值变了 (求和)，形状不变 | 形状变大 (拼接)，内容变全      | 形状不变 (总量)，内容重排        |
| **主要用途** | 数据并行 (梯度同步)   | 张量并行 / 序列并行         | MoE / 高级序列并行          |
| **通信量级** | $O(M)$ (较小)   | $O(N \times M)$ (大) | $O(N \times M)$ (大且乱) |
| **性能杀手** | ⭐⭐            | ⭐⭐⭐⭐                | ⭐⭐⭐⭐⭐ (最难优化)          |
